...
//   마이페이지
   	public MemberVO findMyId() {
//   	단, 패스워드는 *절대 화면으로 가져오면 안된다
   		String qurey = "SELECT ID, MEMBER_EMAIL, MEMBER_PASSWORD, MEMBER_ADDRESS, MEMBER_RECOMMENDER_EMAIL "
   				+ "FROM TBL_MEMBER "
   				+ "WHERE ID = ?";
   		...
   		try {
			...
			memberVO.setMemberPassword(resultSet.getNString("MEMBER_PASSWORD"));
			memberVO.setMemberAddress(resultSet.getNString("MEMBER_ADDRESS"));
			memberVO.setMemberRecommenderEmail(resultSet.getNString("MEMBER_RECOMMENDER_EMAIL"));
...
// 내가 추천한 추천인 조회
 public MemberVO getMyRecommender() {
	   String query = "SELECT ID, MEMBER_EMAIL, MEMBER_ADDRESS "
	         + "FROM TBL_MEMBER "
	         + "WHERE MEMBER_EMAIL = ("
	         + "   SELECT MEMBER_RECOMMENDER_EMAIL FROM TBL_MEMBER WHERE ID = ?"
	         + ")";
